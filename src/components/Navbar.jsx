    ...
    <nav className="bg-white border-b border-gray-100 sticky top-0 z-50 shadow-sm">
      <div className="max-w-6xl mx-auto px-6 py-3 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-3">
          {user ? (
            <>
              <img src="https://i.pravatar.cc/100" alt="profile" className="w-9 h-9 rounded-full border-2 border-sky-500" />
              <button className="text-sm px-5 py-2 rounded-full border border-sky-500 text-sky-500 hover:bg-sky-50">
                Logout
              </button>
            </>
          ) : (
            <>
              <Link href="/login" className="text-sm px-5 py-2 rounded-full border border-gray-400 text-gray-600 hover:border-sky-500 hover:text-sky-500">
                Login
              </Link>
              <Link href="/register" className="text-sm px-5 py-2 rounded-full bg-sky-500 text-white hover:bg-sky-600">
                Register
              </Link>
            </>
          )}
      ...
      {menuOpen && (
        <div className="md:hidden bg-white border-t border-gray-100 px-6 py-4 flex flex-col gap-4">
          ...
          <div className="flex gap-3">
            ...
            <Link href="/register" className="text-sm px-5 py-2 rounded-full bg-sky-500 text-white">Register</Link>
          </div>
        </div>
      )}
    </nav>
