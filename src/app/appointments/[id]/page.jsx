...
export default function DoctorDetails() {
  ...
  const doctorId = params?.id; // dynamic URL থেকে ID নেওয়া
  ...
  // 🔄 কারেন্ট লগইন থাকা ইউজারের সেশন ডাটা
  const { data: session } = authClient.useSession();

  // 🔄 URL এর ID অনুযায়ী ডক্টরের ডাটা ব্যাকএন্ড থেকে নিয়ে আসা
  useEffect(() => {
    if (!doctorId) return;
    fetch(`https://docappoint-server-ewq6.onrender.com/doctors/${doctorId}`)
      .then((res) => res.json())
      .then((data) => {
        setDoctor(data);
  ...
  const handleBooking = async (e) => {
    ...
    const bookingData = {
      ...
      patientName: session.user.name,      // লগইন থাকা ইউজারের নাম
      userEmail: session.user.email,       // লগইন থাকা ইউজারের ইমেইল
      appointmentDate,
      timeSlot,
      status: "Pending",
    };
      ...
      <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 mt-8">
        ...
        {!session ? (
          <div className="text-center p-4 bg-amber-50 rounded-xl text-amber-700 font-medium">
            Please{" "}
            <span 
              className="underline cursor-pointer font-bold text-sky-500" 
              onClick={() => router.push(`/login?redirect=/appointments/${doctorId}`)}
            >
              Login
            </span>{" "}
          ...
          <form onSubmit={handleBooking} className="space-y-4">
            ...
            <button 
              type="submit" 
              disabled={loading}
              className="btn bg-slate-950 text-white w-full rounded-xl hover:bg-slate-900 mt-4 h-12"
            >
